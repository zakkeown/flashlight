...
class Dirichlet(ExponentialFamily):
    ...
    def __init__(
        self, concentration: Union[Tensor, mx.array], validate_args: Optional[bool] = None
    ):
        self.concentration = (
    ...
    def mode(self) -> Tensor:
        ...
        return Tensor(
            ...
            / (mx.sum(self.concentration, axis=-1, keepdims=True) - self.concentration.shape[-1])
        )
    ...
    def entropy(self) -> Tensor:
        ...
        return Tensor(
            log_beta
            + (a0 - k) * digamma_sum
            - mx.sum((self.concentration - 1) * digamma_each, axis=-1)
        )
