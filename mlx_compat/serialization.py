...
def save(
    ...
    Example:
        ...
        >>> mlx_compat.save(model.state_dict(), 'model.pt')
        >>>
        >>> # Save a tensor
        >>> x = mlx_compat.randn(10, 20)
        >>> mlx_compat.save(x, 'tensor.pt')
        >>>
        >>> # Save a dictionary
        >>> checkpoint = {'epoch': 10, 'model': model.state_dict(), 'optimizer': optimizer.state_dict()}
        >>> mlx_compat.save(checkpoint, 'checkpoint.pt')
    """
...
def load(
    ...
    **pickle_load_args
    ...
    Loads an object that was saved with mlx_compat.save() or torch.save().
    ...
    Example:
        ...
        >>> state_dict = mlx_compat.load('model.pt')
        >>> model.load_state_dict(state_dict)
        >>>
        >>> # Load a tensor
        >>> x = mlx_compat.load('tensor.pt')
        >>>
        >>> # Load a checkpoint
        >>> checkpoint = mlx_compat.load('checkpoint.pt')
