...
class LogisticNormal(Distribution):
    ...
    def sample(self, sample_shape: Tuple[int, ...] = ()) -> Tensor:
        ...
        return Tensor(mx.softmax(mx.concatenate([x, mx.zeros(shape[:-1] + (1,))], axis=-1), axis=-1))
    ...
    def log_prob(self, value: Tensor) -> Tensor:
        ...
        # Normal log prob
        import math
        log_prob = -0.5 * mx.sum(((x - self.loc) / self.scale) ** 2 + mx.log(2 * math.pi * self.scale ** 2), axis=-1)
        # Jacobian correction
