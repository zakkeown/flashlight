...
class TestDtype(TestCase):
    ...
    def test_dtype_attributes(self):
        ...
        self.assertIsNotNone(flashlight.float32)
        self.assertIsNotNone(flashlight.float16)
        self.assertIsNotNone(flashlight.int32)
        self.assertIsNotNone(flashlight.int64)
        self.assertIsNotNone(flashlight.bool)

    def test_dtype_aliases(self):
        """Test dtype aliases."""
        self.assertEqual(flashlight.float, flashlight.float32)
        self.assertEqual(flashlight.half, flashlight.float16)
        self.assertEqual(flashlight.int, flashlight.int32)
        self.assertEqual(flashlight.long, flashlight.int64)
    ...
    def test_dtype_conversion_int(self):
        ...
        t = flashlight.tensor([1.5, 2.7, 3.2])
        t_int = t.int()
        self.assertEqual(t_int.dtype, flashlight.int32)

    def test_dtype_conversion_long(self):
        """Test dtype conversion to long."""
        t = flashlight.tensor([1, 2, 3])
        t_long = t.long()
        self.assertEqual(t_long.dtype, flashlight.int64)

    def test_dtype_conversion_half(self):
        """Test dtype conversion to half."""
        t = flashlight.tensor([1.0, 2.0, 3.0])
        t_half = t.half()
        self.assertEqual(t_half.dtype, flashlight.float16)

    def test_dtype_conversion_bool(self):
        """Test dtype conversion to bool."""
        t = flashlight.tensor([0, 1, 2])
        t_bool = t.bool()
        self.assertEqual(t_bool.dtype, flashlight.bool)

    def test_to_dtype(self):
        """Test .to() with dtype."""
        t = flashlight.tensor([1, 2, 3])
        t_int = t.to(dtype=flashlight.int32)
        self.assertEqual(t_int.dtype, flashlight.int32)

    def test_type_method(self):
        """Test .type() method."""
        t = flashlight.tensor([1.0, 2.0, 3.0])
        t_int = t.type(flashlight.int32)
        self.assertEqual(t_int.dtype, flashlight.int32)
...
class TestDtypeFunctions(TestCase):
    ...
    def test_get_default_dtype(self):
        ...
        dtype = flashlight.get_default_dtype()
        self.assertEqual(dtype, flashlight.float32)

    def test_set_default_dtype(self):
        """Test set_default_dtype function."""
        original = flashlight.get_default_dtype()
        try:
            flashlight.set_default_dtype(flashlight.float16)
            self.assertEqual(flashlight.get_default_dtype(), flashlight.float16)
        finally:
            flashlight.set_default_dtype(original)

    def test_is_floating_point(self):
        """Test is_floating_point function."""
        float_t = flashlight.tensor([1.0, 2.0])
        int_t = flashlight.tensor([1, 2], dtype=flashlight.int32)
        self.assertTrue(flashlight.is_floating_point(float_t))
        self.assertFalse(flashlight.is_floating_point(int_t))

    def test_is_complex(self):
        """Test is_complex function."""
        real_t = flashlight.tensor([1.0, 2.0])
        self.assertFalse(flashlight.is_complex(real_t))
...
class TestTensorConversions(TestCase):
    ...
    def test_item_single_element(self):
        ...
        t = flashlight.tensor([42])
        val = t.item()
        self.assertEqual(val, 42)

    def test_item_scalar(self):
        """Test .item() for scalar tensor."""
        t = flashlight.tensor(3.14)
        val = t.item()
...
class TestOperatorOverloading(TestCase):
    ...
    def test_sub(self):
        ...
        a = flashlight.tensor([5, 6, 7])
        b = flashlight.tensor([1, 2, 3])
        c = a - b
