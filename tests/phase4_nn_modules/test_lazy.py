...
class TestLazyLinear(TestCase):
    ...
    def test_creation(self):
        ...
        linear = mlx_compat.nn.LazyLinear(out_features=64)
    ...
    def test_forward_infers_in_features(self):
        ...
        linear = mlx_compat.nn.LazyLinear(out_features=64)
        x = mlx_compat.randn(4, 32)  # 32 input features
        output = linear(x)
...
class TestLazyConv1d(TestCase):
    ...
    def test_creation(self):
        ...
        conv = mlx_compat.nn.LazyConv1d(out_channels=32, kernel_size=3)
    ...
    def test_forward_infers_in_channels(self):
        ...
        conv = mlx_compat.nn.LazyConv1d(out_channels=32, kernel_size=3, padding=1)
        x = mlx_compat.randn(4, 16, 100)  # batch, in_channels, length
        output = conv(x)
    ...
    def test_with_stride(self):
        ...
        conv = mlx_compat.nn.LazyConv1d(out_channels=32, kernel_size=3, stride=2)
        x = mlx_compat.randn(4, 16, 100)
        output = conv(x)
...
class TestLazyConv2d(TestCase):
    ...
    def test_creation(self):
        ...
        conv = mlx_compat.nn.LazyConv2d(out_channels=32, kernel_size=3)
        self.assertEqual(conv.out_channels, 32)

    def test_forward_infers_in_channels(self):
        """Test LazyConv2d forward pass infers input channels."""
        conv = mlx_compat.nn.LazyConv2d(out_channels=32, kernel_size=3, padding=1)
        x = mlx_compat.randn(4, 16, 8, 8)  # batch, in_channels, height, width
        output = conv(x)
    ...
    def test_with_stride_and_padding(self):
        ...
        conv = mlx_compat.nn.LazyConv2d(out_channels=32, kernel_size=3, stride=2, padding=1)
        x = mlx_compat.randn(4, 16, 32, 32)
        output = conv(x)
...
class TestLazyConv3d(TestCase):
    ...
    def test_creation(self):
        ...
        conv = mlx_compat.nn.LazyConv3d(out_channels=32, kernel_size=3)
        self.assertEqual(conv.out_channels, 32)

    def test_forward_infers_in_channels(self):
        """Test LazyConv3d forward pass infers input channels."""
        conv = mlx_compat.nn.LazyConv3d(out_channels=32, kernel_size=3, padding=1)
        x = mlx_compat.randn(2, 8, 4, 4, 4)  # batch, in_channels, depth, height, width
        output = conv(x)
...
class TestLazyConvTranspose1d(TestCase):
    ...
    def test_creation(self):
        ...
        conv = mlx_compat.nn.LazyConvTranspose1d(out_channels=32, kernel_size=3)
        self.assertEqual(conv.out_channels, 32)

    def test_forward_infers_in_channels(self):
        """Test LazyConvTranspose1d forward pass infers input channels."""
        conv = mlx_compat.nn.LazyConvTranspose1d(out_channels=32, kernel_size=3, padding=1)
        x = mlx_compat.randn(4, 16, 100)
        output = conv(x)
        self.assertEqual(output.shape[0], 4)
        self.assertEqual(output.shape[1], 32)
...
class TestLazyConvTranspose2d(TestCase):
    ...
    def test_creation(self):
        ...
        conv = mlx_compat.nn.LazyConvTranspose2d(out_channels=32, kernel_size=3)
        self.assertEqual(conv.out_channels, 32)

    def test_forward_infers_in_channels(self):
        """Test LazyConvTranspose2d forward pass infers input channels."""
        conv = mlx_compat.nn.LazyConvTranspose2d(out_channels=32, kernel_size=3, padding=1)
        x = mlx_compat.randn(4, 16, 8, 8)
        output = conv(x)
        self.assertEqual(output.shape[0], 4)
        self.assertEqual(output.shape[1], 32)
        self.assertEqual(conv.in_channels, 16)
...
class TestLazyConvTranspose3d(TestCase):
    ...
    def test_creation(self):
        ...
        conv = mlx_compat.nn.LazyConvTranspose3d(out_channels=32, kernel_size=3)
        self.assertEqual(conv.out_channels, 32)

    def test_forward_infers_in_channels(self):
        """Test LazyConvTranspose3d forward pass infers input channels."""
        conv = mlx_compat.nn.LazyConvTranspose3d(out_channels=32, kernel_size=3, padding=1)
        x = mlx_compat.randn(2, 8, 4, 4, 4)
        output = conv(x)
...
class TestLazyBatchNorm1d(TestCase):
    ...
    def test_2d_input(self):
        ...
        bn = mlx_compat.nn.LazyBatchNorm1d()
        x = mlx_compat.randn(4, 32)  # batch, features
        output = bn(x)
...
class TestLazyBatchNorm2d(TestCase):
    ...
    def test_creation(self):
        ...
        bn = mlx_compat.nn.LazyBatchNorm2d()
        self.assertIsNotNone(bn)

    def test_forward_infers_num_features(self):
        """Test LazyBatchNorm2d forward pass infers num_features."""
        bn = mlx_compat.nn.LazyBatchNorm2d()
        x = mlx_compat.randn(4, 32, 8, 8)  # batch, channels, height, width
        output = bn(x)
        self.assertEqual(output.shape, (4, 32, 8, 8))
        self.assertEqual(bn.num_features, 32)
...
class TestLazyBatchNorm3d(TestCase):
    ...
    def test_creation(self):
        ...
        bn = mlx_compat.nn.LazyBatchNorm3d()
        self.assertIsNotNone(bn)

    def test_forward_infers_num_features(self):
        """Test LazyBatchNorm3d forward pass infers num_features."""
        bn = mlx_compat.nn.LazyBatchNorm3d()
        x = mlx_compat.randn(2, 16, 4, 4, 4)  # batch, channels, depth, height, width
        output = bn(x)
...
class TestLazyInstanceNorm1d(TestCase):
    ...
    def test_creation(self):
        ...
        norm = mlx_compat.nn.LazyInstanceNorm1d()
        self.assertIsNotNone(norm)

    def test_forward_infers_num_features(self):
        """Test LazyInstanceNorm1d forward pass infers num_features."""
        norm = mlx_compat.nn.LazyInstanceNorm1d()
        x = mlx_compat.randn(4, 32, 100)
        output = norm(x)
        self.assertEqual(output.shape, (4, 32, 100))
...
class TestLazyInstanceNorm2d(TestCase):
    ...
    def test_creation(self):
        ...
        norm = mlx_compat.nn.LazyInstanceNorm2d()
        self.assertIsNotNone(norm)
...
class TestLazyInstanceNorm3d(TestCase):
    ...
    def test_creation(self):
        ...
        norm = mlx_compat.nn.LazyInstanceNorm3d()
        self.assertIsNotNone(norm)
